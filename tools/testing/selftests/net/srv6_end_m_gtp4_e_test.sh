...
# srupf IPv6 source addresses exercise a /64, /48 and /96 Source UPF
# Prefix, and a nonzero ignored tail.
#
...
#
# Cases exercised:
#   1. SRH absent, v6_src_prefix_len=64, pdu_type=dl
#   2. SRH present (H.Encaps from srupf)
#   3. v6_src_prefix_len=48
#   4. v6_src_prefix_len=96 (32-bit window at the very end)
...
#   9. Bad or missing attribute -- ip route add returns EINVAL
...
test_default()
{
	...
	rc=$(run_recv_match "${SID_DEFAULT}" "${SRC6_DEFAULT}" "${V4_SRC}" \
		--pdu-session)
	log_test "$rc" 0 "End.M.GTP4.E SRH-less, v6_src_prefix_len=64, PDU Session"
}
...
test_bad_attrs()
{
	...
	# v6_src_prefix_len omitted.  The Source UPF Prefix length has to
	# match the node that writes the IPv6 source, so there is no value
	# the kernel could pick on its own.
...
test_gso()
{
	local srupf_mac ing_pid cnt_pid cap ing_err cnt_err seg_count ing_rc=0 rc=0
