...
run_test()
{
	...
	local expected_srh0_locator="$2"	# locator+function of last SID of B
	...
	# must reach the upf.  Per RFC 9433 Section 6.3 S04+S08 the SRv6
	# packet pushed by End.M.GTP6.D carries the SR Policy B verbatim
	# (no preserved D — that is End.M.GTP6.D.Di Section 6.4) and writes
	# Args.Mob.Session into SRH[0] (the last SID of B in path order =
	# segments[0] in the kernel's array).  The first SID of B is the
	# initial outer DA and lands at SRH[N-1].
	EXPECTED_SRH0_LOC="$expected_srh0_locator" python3 - "$out" <<'PYEOF'
import ipaddress, os, sys
from scapy.all import rdpcap, IPv6, IPv6ExtHdrSegmentRouting

expected_loc = os.environ['EXPECTED_SRH0_LOC']
expected_loc_packed = ipaddress.IPv6Address(expected_loc).packed
pkts = rdpcap(sys.argv[1])
for p in pkts:
    ...
    # /64 locator must match the configured last SID of B.
    if srh0[:8] != expected_loc_packed[:8]:
        zeros = b"\0" * 8
        got = ipaddress.IPv6Address(srh0[:8] + zeros)
        sys.exit(f"SRH[0] locator = {got} (want {expected_loc} at /64)")
    # Args.Mob.Session stamps bytes 8..12 of the SID (sr_prefix_len=64).
    args = srh0[8:13]
    if args != bytes.fromhex("1400000123"):
        sys.exit(f"Args.Mob.Session at SRH[0] tail = {args.hex()} "
                 f"(want 1400000123)")
    sys.exit(0)
...
run_nf_test()
{
	...
	local expect_srh0_loc="$2"	# /64 locator of last SID, empty when no packet expected
	...
	if [ -n "$expect_srh0_loc" ]; then
		EXPECTED_SRH0_LOC="$expect_srh0_loc" python3 - "$out" <<'PYEOF'
import ipaddress, os, sys
from scapy.all import rdpcap, IPv6, IPv6ExtHdrSegmentRouting

expected_loc = ipaddress.IPv6Address(os.environ['EXPECTED_SRH0_LOC']).packed
pkts = rdpcap(sys.argv[1])
for p in pkts:
    if not (IPv6 in p and IPv6ExtHdrSegmentRouting in p):
        continue
...
main()
{
	...
	# default: srh segs 2001:db8:2::e,2001:db8:3::e — last SID of B is
	# 2001:db8:3::e (per RFC 9433 Section 6.3 SRH[0] = last SID).
	if run_test "2001:db8:f::1" "2001:db8:3::"; then
	...
	if [ "$have_vrf" = "1" ]; then
		# vrf case: srh segs 2001:db8:4::e,2001:db8:5::e
		if run_test "2001:db8:f0::1" "2001:db8:5::" \
	...
	if command -v nft >/dev/null && \
	   ip netns exec "$srgw" sysctl -wq \
		...
		if run_nf_test accept "2001:db8:3::"; then
