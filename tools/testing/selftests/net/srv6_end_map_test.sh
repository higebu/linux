...
#                                  (End.MAP)
#
# srupf2 holds the End.MAP route for 2001:db8:f::/64 that replaces the
# IPv6 destination with 2001:db8:3::3 (an address on srupf3's loopback,
...
#                        before forwarding so the packet never
#                        reaches srupf3.
...
setup()
{
	...
	ip link add veth1 netns "$srupf1" \
		type veth peer name veth1-srupf2 netns "$srupf2"
	ip link add veth2 netns "$srupf2" \
		type veth peer name veth2-srupf3 netns "$srupf3"

	ip -n "$srupf1" addr add 2001:db8:1::1/64 dev veth1 nodad
	ip -n "$srupf2" addr add 2001:db8:1::2/64 dev veth1-srupf2 nodad
	ip -n "$srupf2" addr add 2001:db8:2::1/64 dev veth2 nodad
	ip -n "$srupf3" addr add 2001:db8:2::2/64 dev veth2-srupf3 nodad
	# srupf3 also owns the End.MAP replacement SID / SRH endpoint.
	ip -n "$srupf3" addr add "$SRUPF3_SID/128" dev lo nodad

	ip -n "$srupf1" link set veth1 up
	ip -n "$srupf2" link set veth1-srupf2 up
	ip -n "$srupf2" link set veth2 up
	ip -n "$srupf3" link set veth2-srupf3 up

	ip netns exec "$srupf2" sysctl -wq net.ipv6.conf.all.forwarding=1

	# srupf3 must accept SRv6 packets so ipv6_srh_rcv lets the
	# extension header chain through to local delivery.
	ip netns exec "$srupf3" sysctl -wq net.ipv6.conf.all.seg6_enabled=1
	ip netns exec "$srupf3" \
		sysctl -wq net.ipv6.conf.veth2-srupf3.seg6_enabled=1
	ip netns exec "$srupf3" sysctl -wq net.ipv6.conf.lo.seg6_enabled=1

	# Disable HW checksum offload so the kernel software checksum
	# path runs unconditionally and any csum bug surfaces.
	ip netns exec "$srupf1" ethtool -K veth1 tx off rx off 2>/dev/null
	ip netns exec "$srupf2" ethtool -K veth1-srupf2 tx off rx off \
		2>/dev/null
	ip netns exec "$srupf2" ethtool -K veth2 tx off rx off 2>/dev/null
	ip netns exec "$srupf3" ethtool -K veth2-srupf3 tx off rx off \
		2>/dev/null

	# srupf1: route the End.MAP locator into srupf2.
	ip -n "$srupf1" -6 route add "$END_MAP_PREFIX" via 2001:db8:1::2

	# srupf1: a separate H.Encaps route for the SRH-present scenario,
	# wrapping the inner ICMPv6 echo in an outer IPv6+SRH carrying
	# [End.MAP_SID, SRUPF3_SID].
	ip -n "$srupf1" -6 route add "$SRUPF3_SID/128" via 2001:db8:1::2 \
		encap seg6 mode encap \
		segs "$END_MAP_SID","$SRUPF3_SID" \
		dev veth1

	# srupf2: End.MAP -- swap DA from the End.MAP SID to SRUPF3_SID
	# (an address on srupf3) and forward via the IPv6 FIB.
	ip -n "$srupf2" -6 route add "$END_MAP_PREFIX" \
		encap seg6mobile action End.MAP nh6 "$SRUPF3_SID" \
		dev veth2

	# srupf2: reach SRUPF3_SID (on srupf3's loopback) through the
	# directly connected neighbour 2001:db8:2::2.
	ip -n "$srupf2" -6 route add "$SRUPF3_SID/128" via 2001:db8:2::2

	# srupf3: return route for the ICMPv6 echo reply.
...
test_srh_absent()
{
	...
	if [ "$rc" -eq 0 ]; then
		after=$(read_nstat_counter "$srupf3" Icmp6InCsumErrors)
		[ "$before" != "$after" ] && rc=1
	fi
...
# Test 2: SRH present (H.Encaps from srupf1).
#
# The packet srupf1 emits carries an SRH whose only intermediate
# segment is the End.MAP SID; the final segment is SRUPF3_SID.  End.MAP
# rewrites the outer destination from END_MAP_SID to SRUPF3_SID
# without touching the SRH, so on srupf3 the SRH processing advances
# to segments[0] = SRUPF3_SID (local) and the encapsulated inner
...
test_srh_present()
{
	...
	if ! ip netns exec "$srupf1" \
			ping -6 -c 1 -W "$PING_TIMEOUT_SEC" "$SRUPF3_SID" \
			>/dev/null 2>&1; then
		rc=1
	fi
...
#
# srupf1 inserts an SRH between the IPv6 header and the L4 payload;
# segments[0] is the original DA (2001:db8:2::2), segments[1] is the
# End.MAP SID.  End.MAP rewrites the outer DA to SRUPF3_SID but must
# NOT patch the L4 checksum, because the kernel's standard SRv6
# processing on srupf3 will decrement Segments Left to 0 and restore
...
test_srh_inline()
{
	...
	ip -n "$srupf1" -6 route add 2001:db8:2::2/128 via 2001:db8:1::2 \
...
# rather than 4 (SRH), then sends it from srupf1 toward the End.MAP
# SID.  End.MAP's seg6_mobile_get_srh() must return MALFORMED and the
# handler must drop the packet before forwarding.
#
# The helper's ICMPv6 checksum is computed against the End.MAP SID
# (the original outer destination it uses to address the packet).  If
# End.MAP nonetheless forwards the packet, srupf2 rewrites the outer
# DA to SRUPF3_SID and srupf3's ICMPv6 receive path recomputes the
# checksum against the new DA -- the mismatch shows up as a non-zero
# Icmp6InCsumErrors counter.  A correctly dropped packet leaves the
# counter alone.
test_srh_malformed()
{
	local before after rc=0
