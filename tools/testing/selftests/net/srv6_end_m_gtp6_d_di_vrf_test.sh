...
# Selftest for the SRv6 End.M.GTP6.D.Di behavior with vrftable (RFC 9433).
#
# Topology matches srv6_end_m_gtp6_d_vrf_test.sh: on srgw the egress leg
# veth-n9 is enslaved into vrf-underlay (table 200) and the route to the
# SR Policy locator lives only in that table.  Removing the vrftable
# attribute from the lwtunnel breaks the post-action lookup, proving the
# attribute is needed.
...
# End.M.GTP6.D.Di: outer DA = SR Policy first SID; prepended SRH[0]
# carries the original outer destination (the Di SID).
...
test_vrftable_positive()
{
	...
	ip -n "$srgw" -6 route add "${DI_SID}/128" \
	...
	rc=$(run_tpdu_match)
	log_test "$rc" 0 "End.M.GTP6.D.Di with vrftable steers egress into the underlay VRF"

	ip -n "$srgw" -6 route del "${DI_SID}/128"
}
...
test_vrftable_required()
{
	...
	rc=$(run_tpdu_match)
	log_test "$rc" 2 "End.M.GTP6.D.Di without vrftable fails to reach the underlay"
...
test_strict_mode_off()
{
	...
	log_test "$rc" 0 "End.M.GTP6.D.Di vrftable rejected when strict_mode is off"
}
...
test_bad_vrftable()
{
	...
	log_test "$rc" 0 "End.M.GTP6.D.Di rejects vrftable 0 and unbound table"
}
