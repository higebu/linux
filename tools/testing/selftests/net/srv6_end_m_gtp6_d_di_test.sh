...
# Selftest for the SRv6 End.M.GTP6.D.Di behavior (RFC 9433).
#
...
# Unlike End.M.GTP6.D, the GTP-U TEID/QFI are discarded;
...
#   6. Fragmented outer IPv6 -> drop
#   7. Duplicate PDU Session containers -> drop
#   8. GTPv1-U Echo Request -> passthrough to lupf
#   9. Bad attribute (missing srh) -> ip route add returns EINVAL
...
# Args.Mob.Session layout: QFI(6) | R(1) | U(1) | TEID(32).  For
# TEID=0x123, QFI=0 the 40-bit stamp is 0x00_00_00_01_23; with
# sr_prefix_len=64 it lands at bytes 8..12 of SRH[0].
...
setup()
{
	...
	# End.M.GTP6.D on the same srgw exposes the .D wire format for
...
test_compare_with_d()
{
	...
	# Under End.M.GTP6.D the same input yields SRH[0] carrying the
	# Args.Mob.Session stamp instead of the original SID, so a swap
	# between the two action handlers would surface as a mismatch here.
...
test_drop_frag_outer()
{
	...
	rc=$(run_tpdu_no_match --frag --pdu-session \
		-q "${QFI}" -P "${PDU_TYPE_DL}")
	log_drop_test "$rc" "End.M.GTP6.D.Di drops a fragmented outer packet"
}
...
test_drop_dup_pdu_session()
{
	...
	rc=$(run_tpdu_no_match --pdu-session --dup-pdu-session \
		-q "${QFI}" -P "${PDU_TYPE_DL}")
	log_drop_test "$rc" "End.M.GTP6.D.Di drops duplicate PDU Session containers"
}
