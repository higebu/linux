...
# (RFC 9433 Section 6.5) packs Args.Mob.Session (40 bits: QFI(6) | R(1)
# | U(1) | TEID(32)) at bytes 8..12, giving the SID
# 2001:db8:e::1400:1:2300:0 for QFI=5 / TEID=0x00000123.
#
...
# mandatory (RFC 8200 Section 8.1); Udp6InCsumErrors must remain zero.
#
# Cases exercised:
...
# the window also fails the case (RFC 8200 Section 8.1 mandates a
# non-zero UDP/IPv6 checksum).
run_recv_match()
{
	local trigger="$1"
	local extra=("${@:2}")
