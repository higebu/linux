					    ...
					    struct sk_buff *skb)
{
	...
	/* RFC 9433 Section 6.3 S04: "Push a new IPv6 header with its own
	 * SRH containing B".  The configured SR Policy is pushed verbatim
	 * — unlike End.M.GTP6.D.Di (Section 6.4 S08), End.M.GTP6.D does
	 * NOT prepend the original outer DA D.  The original D is dropped
	 * with the GTP-U envelope.
	 */
	...
	new_srh = (struct ipv6_sr_hdr *)(skb_network_header(skb) +
					 sizeof(struct ipv6hdr));
	/* RFC 9433 Section 6.3 S08: Write Args.Mob.Session into SRH[0]
	 * (the last SID in path order — the egress endpoint that handles
	 * the per-session identifiers).
	 */
	if (seg6_mobile_write_args_mob(&new_srh->segments[0],
	 ...
	 * DA before Args.Mob.Session was stamped; for a single-segment SR
	 * Policy first_segment == 0 and the slot just got rewritten,
	 * so refresh outer DA.  For multi-segment policies first_segment
	 * != 0 so this is a no-op (the first SID is unchanged), matching
	 * RFC 9433 Section 6.3 S06 ("Set the outer IPv6 DA to the first
	 * SID of B").
	 */
 ...
 * Args.Mob.Session, which per Section 6.3 Step S08 ("Write in the
 * SRH[0] the Args.Mob.Session") is written into segments[0] of the
 * new SRH -- the last SID in path order, the egress endpoint that
 * processes the per-session identifiers.  The original outer IPv6
 * DA is dropped together with the GTP-U envelope.
 *
 * When net.netfilter.nf_hooks_lwtunnel=1 the inner T-PDU is exposed
...
/* End.M.GTP6.D (RFC 9433 Section 6.3): the configured SR Policy is
 * pushed onto the packet as-is -- no extra leading slot, no original-D
 * preservation.  Only validate the SRH is present and well-formed.
 */
static int seg6_end_m_gtp6_d_build(struct seg6_local_lwt *slwt,
				   const void *cfg,
				   struct netlink_ext_ack *extack)
{
	if (!slwt->srh) {
		NL_SET_ERR_MSG_MOD(extack,
				   "End.M.GTP6.D requires srh segs");
		return -EINVAL;
	}
	if (slwt->srh->hdrlen > 254) {
		NL_SET_ERR_MSG_MOD(extack,
				   "End.M.GTP6.D srh too large (max 127 segments)");
		return -EINVAL;
	}
	return 0;
}

/* End.M.GTP6.D.Di (RFC 9433 Section 6.4): the SRH must reserve a
 * leading slot for the original outer destination D, which the data
 * path stamps in after seg6_do_srh_encap().  Build that augmented SRH
 * up-front so the fast path is allocation-free.
 */
static int seg6_end_m_gtp6_d_di_aug_build(struct seg6_local_lwt *slwt,
					  const void *cfg,
					  struct netlink_ext_ack *extack)
{
...
static void seg6_end_m_gtp6_d_di_aug_destroy(struct seg6_local_lwt *slwt)
{
...
/* Forward declarations; defined next to the parse_nla_mobile_* helpers. */
...
static struct seg6_action_desc seg6_action_table[] = {
	...
	},
	{
		...
		.slwt_ops	= {
			.build_state = seg6_end_m_gtp6_d_build,
		},
	},
	{
		...
		.slwt_ops	= {
			.build_state = seg6_end_m_gtp6_d_di_aug_build,
			.destroy_state = seg6_end_m_gtp6_d_di_aug_destroy,
		},
	},
	{
