					    ...
					    struct sk_buff *skb)
{
	...
	/* Per-route oif lets the operator bind the egress GTP-U flow to a
	 * specific interface or VRF (e.g. a per-route VRF or VLAN
	 * sub-interface).
	 */
				     ...
				     struct seg6_local_lwt *slwt)
{
	...
	/* Initial transport-header offset for netfilter; matches the
	 * pattern used by End.DX4 / End.DX6.  Netfilter walks IPv6
	 * extension headers / IPv4 options as needed.
	 */
	skb_set_transport_header(skb,
				 (inner_proto == IPPROTO_IPIP) ?
				 sizeof(struct iphdr) :
				...
				struct seg6_local_lwt *slwt)
{
	...
	skb_dst_drop(skb);
	/* seg6_lookup_nexthop()'s return is intentionally ignored: even on
	 * a routing failure it installs a black-hole dst on the skb, so the
	 * subsequent dst_input() will drop the packet with the appropriate
	 * IPv4/IPv6 error.  This matches the convention used by every other
	 * seg6_local action handler in this file.
	 */
	seg6_lookup_nexthop(skb, NULL, 0);
	return dst_input(skb);
