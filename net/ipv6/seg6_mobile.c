				       ...
				       __be32 *v4_da, u64 *args_mob)
{
	...
	if (locator_bits + 32 + SEG6_MOBILE_ARGS_MOB_LEN > 128)
		return false;
				   ...
				   u8 outer_hoplimit, __be32 flowlabel)
{
	 ...
	 * sum so the outer UDPv6 checksum can be completed downstream.  For a
	 * GSO skb the inner CHECKSUM_PARTIAL offload must stay intact, so only
	 * the seed is written here and skb_udp_tunnel_segment() folds the
	 * outer checksum per segment; for a non-GSO skb CHECKSUM_PARTIAL hands
	 * the outer checksum to the stack or NIC.
	 */
				     ...
				     struct seg6_mobile_lwt *slwt)
{
	...
	ip6h = ipv6_hdr(skb);
	locator_bits = slwt->sr_prefix_len;
...
static int seg6_mobile_fill_egress_sid(struct in6_addr *sid,
				       ...
				       __be32 v4, u64 args)
{
	if (prefix_bits + 32 + SEG6_MOBILE_ARGS_MOB_LEN > 128)
		return -EINVAL;
				   ...
				   struct netlink_ext_ack *extack)
{
	...
	if ((unsigned int)slwt->sr_prefix_len + 32 +
	    SEG6_MOBILE_ARGS_MOB_LEN > 128) {
		NL_SET_ERR_MSG_MOD(extack,
