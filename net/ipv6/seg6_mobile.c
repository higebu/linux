...
/* End.MAP: replace the IPv6 destination with the configured next SID
 * and forward.  The SRH (if any) is preserved so downstream segments
 * still observe the original list; a malformed SRH is dropped here
...
/* End.M.GTP4.E: receive an SRv6 packet whose IPv6 destination encodes
 * an IPv4 DA + 40-bit Args.Mob.Session, and re-encapsulate the popped
 * inner T-PDU in IPv4/UDP/GTP-U (with an optional PDU Session
 * extension header) toward a legacy IPv4 receiver.  DSCP/ECN are
 * copied from the IPv6 traffic class to the IPv4 ToS, and Hop Limit
 * to TTL.
 */
				     ...
				     struct seg6_mobile_lwt *slwt)
{
	...
	/* Pop the IPv6 header and all its extension headers.
	 * ipv6_skip_exthdr() walks HBH / Routing / Dest-Opts / Fragment
	 * in addition to the SRH.
	 */
	nh = ip6h->nexthdr;
 ...
 * extension headers must already have been popped.  UDP/IPv6
 * checksums are mandatory (RFC 8200 Section 8.1); the pseudo-header
 * sum is seeded here and CHECKSUM_PARTIAL is used so the stack or
 * NIC can complete it.
 */
				   ...
				   __be32 flowlabel)
{
	...
	/* RFC 8200 Section 8.1 requires the UDP checksum over IPv6 to be
	 * non-zero.  Seed the pseudo-header sum and let the stack or the
	 * NIC complete it through CHECKSUM_PARTIAL; if the linear path
	 * folds the checksum to zero, udp6_set_csum() substitutes 0xffff.
	 */
...
/* End.M.GTP6.E: receive an SRv6 packet whose current SID encodes a
 * 40-bit Args.Mob.Session and whose SRH carries the next segment in
 * SRH[0], and re-encapsulate the popped inner T-PDU in IPv6/UDP/GTP-U
 * (with an optional PDU Session extension header) toward that next
 * segment.  Traffic class, flow label and hop limit are propagated
 * from the inbound IPv6 outer per RFC 6040.
 */
				     ...
				     struct seg6_mobile_lwt *slwt)
{
	...
	/* RFC 9433 Section 6.5 SRH-S02: an End.M.GTP6.E SID is always
	 * the penultimate segment, so it becomes the active SID only
