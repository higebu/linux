				    ...
				    u8 nexthdr)
{
	...
	case IPPROTO_UDP: {
		...
		/* zero UDPv6 checksum on a non-offloaded skb means "none" */
				  ...
				  const struct in6_addr *nh, bool srh_present)
{
	...
	if (!srh_present) {
		...
		/* only the first fragment carries the L4 header; frag_off
		 * is the raw field, M flag included
		 */
 ...
 * from the inbound IPv6 SA at bit offset @v6_src_prefix_len.  The
 * trailing bits are "any bit pattern (ignored)" and are not read.
 */
				   ...
				   u8 outer_hoplimit, __be32 flowlabel)
{
	 ...
	 * let skb_udp_tunnel_segment() complete it per segment; with
	 * udp6_zerocsum_tx the plain UDP tunnel type keeps the zero checksum
	 * in every segment instead (RFC 6935).  Non-GSO T-PDUs need none of
	 * this and are shipped as-is.
	 */
	if (inner_proto && skb_is_gso(skb)) {
		if (inner_proto == htons(ETH_P_IP)) {
			if (!pskb_may_pull(skb, sizeof(struct iphdr)))
		...
		skb_set_transport_header(skb, inner_nhlen);
		skb_reset_mac_header(skb);
		err = iptunnel_handle_offloads(skb, slwt->udp6_zerocsum_tx ?
					       SKB_GSO_UDP_TUNNEL :
	...
	/* UDP checksum over IPv6 must be non-zero unless the operator opted
	 * into zero-checksum transmit (RFC 6935), in which case
	 * udp6_set_csum() clears it.  Otherwise it seeds the pseudo-header
	 * sum: a GSO skb keeps its inner CHECKSUM_PARTIAL offload intact and
	 * skb_udp_tunnel_segment() folds the outer checksum per segment; a
	 * non-GSO skb hands the outer checksum to the stack or NIC via
	 * CHECKSUM_PARTIAL (or resolves an inner CHECKSUM_PARTIAL via local
	 * checksum offload).
	 */
	udp6_set_csum(slwt->udp6_zerocsum_tx, skb, v6_sa, v6_da, len);
 ...
 *
 * @route_tunsrc is the operator-configured src_addr; passing it through
 * to seg6_do_srh_encap_tunsrc() skips the RFC 6724 source address
 * selection that every D-family caller immediately overwrites anyway.
 */
				   ...
				   struct in6_addr *route_tunsrc)
{
	...
	err = seg6_do_srh_encap_tunsrc(skb, srh, inner_proto, route_tunsrc);
	if (err)
		return err;
				     ...
				     struct seg6_mobile_lwt *slwt)
{
	 ...
	 * different segment and remains valid.  The outer source address
	 * was already stamped by seg6_mobile_srh_reencap().
	 */
					...
					struct seg6_mobile_lwt *slwt)
{
	 ...
	 * The outer source address was already stamped by
	 * seg6_mobile_srh_reencap().
	 */
 ...
 * @addr at bit offset @v6_src_prefix_len, per RFC 9433.  The trailing
 * "ignored" bits of @addr (the operator's configured src_addr) are left
 * untouched.  seg6_mobile_v4_validate() guarantees the overlay fits in
 * 128 bits at build time.
 */
static void seg6_mobile_overlay_v4(struct in6_addr *addr,
				   u8 v6_src_prefix_len, __be32 v4)
{
	seg6_mobile_addr_set_bits(addr->s6_addr, v6_src_prefix_len, 32,
				  (u64)ntohl(v4) << 32);
}
 ...
 *  - the IPv4 source template, anchored at the Source UPF Prefix length
 *    v6_src_prefix_len, needs that many bits plus 32;
...
static int seg6_mobile_v4_validate(struct seg6_mobile_lwt *slwt,
				   struct netlink_ext_ack *extack)
{
	if (slwt->v6_src_prefix_len + 32 > 128) {
		NL_SET_ERR_MSG_MOD(extack,
...
static int parse_nla_mapped_sid(struct nlattr **attrs,
				struct seg6_mobile_lwt *slwt,
				struct netlink_ext_ack *extack)
{
...
static int cmp_nla_mapped_sid(struct seg6_mobile_lwt *a,
...
static int parse_nla_udp6_zerocsum_tx(struct nlattr **attrs,
				      struct seg6_mobile_lwt *slwt,
				      struct netlink_ext_ack *extack)
{
	slwt->udp6_zerocsum_tx = true;
	return 0;
}

static int put_nla_udp6_zerocsum_tx(struct sk_buff *skb,
				    struct seg6_mobile_lwt *slwt)
{
	if (nla_put_flag(skb, SEG6_MOBILE_UDP6_ZEROCSUM_TX))
		return -EMSGSIZE;
	return 0;
}

static int cmp_nla_udp6_zerocsum_tx(struct seg6_mobile_lwt *a,
				    struct seg6_mobile_lwt *b)
{
	return a->udp6_zerocsum_tx != b->udp6_zerocsum_tx;
}
...
static const struct seg6_mobile_action_desc seg6_mobile_action_table[] = {
	...
	},
	{
		...
		.optattrs	= SEG6_F_MOBILE_COUNTERS |
				  SEG6_MOBILE_F_ATTR(SEG6_MOBILE_PDU_TYPE) |
				  SEG6_MOBILE_F_ATTR(SEG6_MOBILE_UDP6_ZEROCSUM_TX) |
				  SEG6_MOBILE_F_ATTR(SEG6_MOBILE_VRFTABLE),
...
seg6_mobile_action_params[SEG6_MOBILE_MAX + 1] = {
	...
	[SEG6_MOBILE_UDP6_ZEROCSUM_TX] = {
		.parse	= parse_nla_udp6_zerocsum_tx,
		.put	= put_nla_udp6_zerocsum_tx,
		.cmp	= cmp_nla_udp6_zerocsum_tx,
	},
...
seg6_mobile_policy[SEG6_MOBILE_MAX + 1] = {
	...
	[SEG6_MOBILE_UDP6_ZEROCSUM_TX]	= { .type = NLA_FLAG },
...
static int seg6_mobile_get_encap_size(struct lwtunnel_state *lwt)
{
	...
	if (attrs & SEG6_MOBILE_F_ATTR(SEG6_MOBILE_UDP6_ZEROCSUM_TX))
