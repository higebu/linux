...
/* replace the IPv6 destination with the configured next SID and
 * forward; the SRH (if any) is preserved, a malformed SRH is dropped.
 */
...
/* decapsulate the SRv6 outer and emit IPv4/UDP/GTPv1-U toward a legacy
 * IPv4 GTP-U peer using the SID-encoded IPv4 DA / TEID / QFI.
 */
 ...
 * using the configured SR Policy, folding TEID/QFI into Args.Mob.Session
 * stamped into the last SID.  Non-T-PDU GTP-U and non-UDP upper layers
 * fall back to orig_input for control-plane processing downstream.
 */
