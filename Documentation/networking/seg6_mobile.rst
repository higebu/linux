...
The configured SR Policy is pushed verbatim onto the inner T-PDU;
the original outer IPv6 destination ``D`` is dropped together with
the GTP-U envelope.  TEID and QFI are folded into the 40-bit
``Args.Mob.Session`` value defined by RFC 9433 Section 6.1, which
per RFC 9433 Section 6.3 Step S08 ("Write in the SRH[0] the
Args.Mob.Session") is encoded into the locator-relative tail of
``segments[0]`` of the new SRH -- the last SID in path order, the
egress endpoint that processes the per-session identifiers::
...
``sr_prefix_len`` declares the locator length of the SID into which
``Args.Mob.Session`` is encoded and must match the prefix length
configured on the remote SID; the SR Gateway has no way to discover
the remote SID's locator length on its own.

The wire SRH for the example above is a single segment
``2001:db8:2::e`` with ``Args.Mob.Session`` stamped into its
trailing 40 bits.  When the configured SR Policy contains multiple
segments, ``Args.Mob.Session`` is written into ``segments[0]`` (the
last SID in path order) and the remaining segments are pushed as
configured.
